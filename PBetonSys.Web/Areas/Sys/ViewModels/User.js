...
var viewModel = function(){
    ...
    this.grid.OnAfterCreateEditor =function(editors,row){
        if (row._isnew == undefined ) 
            com.readOnlyHandler('input')(editors.ClerkID.target, true);
    };
    this.tree = {
        method:'GET',
    ...
    this.passwordClick = function () {
        ...
        com.message('confirm', '确定要把选中用户的密码重置为<span style="color:red">1234</span>吗？', function (b) {
            if (b) {
                com.ajax({
                    type: 'POST',
                    url: '/api/sys/user/postresetpassword/' + row.ClerkID,
                    success: function () {
...
var setOrganize = function (row) {
    ...
    com.dialog({
        ...
        viewModel:function(w){
            ...
            com.ajax({
                type: 'GET',
                url: '/api/sys/user/getorganizewithusercheck/' + row.ClerkID,
                success: function (d) {
            ...
            this.confirmClick = function () {
                ...
                com.ajax({
                    url: '/api/sys/user/edituserorganizes/' + row.ClerkID,
...
var setRole = function (row) {
    ...
    com.dialog({
        ...
        viewModel: function (w) {
            ...
            com.ajax({
                type: 'GET',
                url: '/api/sys/user/getrolewithusercheck/' + row.ClerkID,
                success: function (d) {
            ...
            this.confirmClick = function () {
                ...
                com.ajax({
                    url: '/api/sys/user/edituserroles/' + row.ClerkID,
    ...
    com.dialog({
        ...
        viewModel: function (w) {
            ...
            this.grid = {
                ...
                pagination: false,
                pageSize: 10,
                url: "/api/sys/user/getsettinglist/" + row.ClerkID,
                queryParams: ko.observable()
            };
            this.cancelClick = function () {
                w.dialog('close');
            ...
            this.grid.toolbar = [
                { text: '新增', iconCls: 'icon-add1', handler: function () { that.gridEdit.addnew({ ClerkID: row.ClerkID }); } }, '-',
            ...
            this.confirmClick = function () {
                ...
                var list = that.gridEdit.getChanges(['ID', 'ClerkID', 'SettingCode', 'SettingName', 'SettingValue', 'Description']);
                com.ajax({
